...
UserProfile::UserProfile()
    : _login("guest"), _user_level(Beginner),
      _times_logged(1), _guesses(0), _correct_guesses(0){
          ...
          /*
           * _itoa()是c标准库所提供的函数，会将整数转换为对应的ASCII字符形式
           * change "_itoa()" ====> to_string()
           */
...
inline void UserProfile::reset_level(const string& level){
    ...
    // assure that level can represetn a user level
    _user_level =
...
std::ostream& operator<<(std::ostream & os, const UserProfile& rhs){
    // output format: stanl Beginner 12 100, 10, 10%
    os << rhs.login() << ' '
       << rhs.level() << ' '
       << rhs.login_count() << ' '
       << rhs.guess_count() << ' '
